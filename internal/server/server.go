...
func NewServer(configFile, version, commit string) (*Server, error) {
	...
	// TEMP: hardcoded for local testing, since config.yaml isn't picked up
	// by hot reload here. Revert to `server.smtpCfg = config.SMTP` only
	// before this ships.
	server.smtpCfg = SMTPConfig{
		Enabled:  true,
		Host:     "smtp.gmail.com",
		Port:     587,
		Username: "tomas@vitvar.com",
		Password: "rhgb tqzt secj rtgl",
